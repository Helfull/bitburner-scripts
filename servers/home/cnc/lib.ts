...
export function progressBar(ns: NS, current: number, max: number, options: ProgressOptions = { width: 20, text: false }) {
  ...
  const leftBar = leftColor.wrap([
    '\uEE04'.repeat(barWidth)
  ].join(''));
  const rightColor = options.reverse ? Color.green : Color.red;
  const rightBar = rightColor.wrap([
    '\uEE01'.repeat(options.width - barWidth),
  ].join(''));

  const startCap = (percent > 0 ? Color.green.wrap('\uEE03') : Color.red.wrap('\uEE00'));
  const endCap = (percent >= 1 ? Color.green.wrap('\uEE05') : Color.red.wrap('\uEE02'));
  const bar = `${startCap}${leftBar}${rightBar}${endCap}`;

  if (options.text === false) return `${bar}`;
  return `${options.formatter(current)}/${options.formatter(max)} (${ns.sprintf('%.2f%%', percent * 100)}) ${bar}`;
}
