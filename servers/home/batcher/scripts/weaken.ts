export async function main(ns: NS) {
  ...
  const weakenResult = await ns.weaken(ns.args[0] as string, {
  ...
  ns.atExit(() => {
    ...
    ns.writePort(signalPort, JSON.stringify({ type: 'weaken', delay, endTime, now: Date.now(), result: weakenResult }));
  }, 'finished-signal');
}