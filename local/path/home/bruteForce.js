...
async function main2(ns) {
  ...
  while (true) {
    let targets = getServers(ns).filter(IS_NOT_PRIVATE(ns)).filter(IS_NOT_HOME(ns)).filter(HAS_ADMIN_ACCESS(ns)).filter(HAS_MONEY(ns)).filter(HAS_MAX_MONEY(ns)).filter(HAS_MIN_SECURITY(ns)).filter(IS_HACKABLE(ns)).filter((s) => ns.getRunningScript("conBatch.js", "home", s) === null).sort(BY_WEIGHT(ns));
    if (targets.length === 0) {
