...
// servers/home/batcher/ContinuesBatcher.ts
var ContinuesBatcher = class extends BatchRunner {
  targettingThreadHandle;
  async loop(target, loop = true) {
    this.log.info("Target: %s", target);
  ...
  async autoTarget() {
    while (true) {
      await this.ns.sleep(100);
      const targets = getServers(this.ns).filter(IS_NOT_PRIVATE(this.ns)).filter(IS_NOT_HOME(this.ns)).filter(HAS_MONEY(this.ns)).filter(IS_GOOD_TARGET(this.ns)).filter(IS_HACKABLE(this.ns)).filter(HAS_MAX_MONEY(this.ns)).filter(HAS_MIN_SECURITY(this.ns)).sort(BY_WEIGHT(this.ns));
      if (targets.length === 0) {
